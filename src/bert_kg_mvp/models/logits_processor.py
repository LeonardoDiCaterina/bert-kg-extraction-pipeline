...
class KGSchemaLogitsProcessor(LogitsProcessor):
    def __init__(self, tokenizer, valid_types, valid_relations):
        ...
        self.valid_relation_first_ids = {tokenizer.encode(r, add_special_tokens=False)[0] for r in valid_relations}
    ...
    def __call__(self, input_ids: torch.LongTensor, scores: torch.FloatTensor) -> torch.FloatTensor:
        ...
        for i in range(batch_size):
            ...
            if self.pipe_id in tokens_list:
                last_pipe_idx = max(loc for loc, val in enumerate(tokens_list) if val == self.pipe_id)
