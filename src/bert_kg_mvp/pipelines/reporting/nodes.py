...
def generate_evaluation_report(
    ...
    report_lines.append("|---|---|")
    report_lines.append(f"| **Ground Truth Triples** | {val_metrics.get('ground_truth', 0)} |")
    report_lines.append(f"| **Extracted Triples** | {val_metrics.get('detected', 0)} |")
    report_lines.append(f"| **True Positive Triples** | {val_metrics.get('tp', 0)} |")
    report_lines.append(f"| **Strict F1 (Exact Match)** | {val_metrics.get('strict_f1', 0.0):.4f} |")
    report_lines.append(f"| **Tuple Precision** | {val_metrics.get('tuple_precision', 0.0):.4f} |")
    report_lines.append(f"| **Tuple Recall** | {val_metrics.get('tuple_recall', 0.0):.4f} |")
    report_lines.append(f"| **Entity Precision** | {val_metrics.get('entity_precision', 0.0):.4f} |")
    report_lines.append(f"| **Entity Recall** | {val_metrics.get('entity_recall', 0.0):.4f} |")
    ...
    for idx in sample_indices:
        ...
        for q in range(outputs["rel_logits"].shape[1]):
            ...
            if pred_r != no_relation_idx:
                ...
                pred_subj = tokenizer.decode(b_ids[0][s_token_indices], skip_special_tokens=True).strip()
                pred_obj = tokenizer.decode(b_ids[0][o_token_indices], skip_special_tokens=True).strip()
                if not pred_subj or not pred_obj:
                    continue
    ...
    for q in range(outputs["rel_logits"].shape[1]):
        ...
        if r != no_relation_idx:
            ...
            sub = tokenizer.decode(b_ids[0][s_token_indices], skip_special_tokens=True).strip()
            obj = tokenizer.decode(b_ids[0][o_token_indices], skip_special_tokens=True).strip()
